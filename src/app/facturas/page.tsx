...
import {
  ...
  Loader2, X, Ban, ArrowLeft, Stamp, Globe, Archive, CheckSquare
} from 'lucide-react';
...
export default function FacturasPage() {
  ...
  const toggleSeleccion = (id: string, e: React.MouseEvent) => {
    ...
    if (seleccionadas.includes(id)) {
      ...
      if (seleccionadas.length >= 10) {
        alert("Máximo 10 facturas a la vez para proteger el rendimiento de tu navegador.");
        return;
      }
  ...
  const handleDescargaMasiva = async () => {
    ...
    try {
      ...
      setSeleccionadas([]); // Limpiar selección tras descargar
    } catch (error) {
  ...
  return (
    <div className="p-8 bg-slate-50 min-h-screen text-slate-800 relative">
      <div className="max-w-7xl mx-auto space-y-6 pb-24">
        ...
        {/* Tabla con Paginación y Checkboxes */}
              ...
              <tbody className="divide-y divide-slate-100">
                {loading ? (
                  ...
                  facturasPaginadas.map(f => (
                    <tr key={f.id} className={`hover:bg-blue-50/50 transition-colors cursor-pointer ${seleccionadas.includes(f.id) ? 'bg-blue-50' : ''}`} onClick={() => setExpandida(expandida === f.id ? null : f.id)}>
      ...
      {seleccionadas.length > 0 && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 bg-slate-800 text-white px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-6 z-50 animate-in slide-in-from-bottom-10 fade-in duration-300 border border-slate-700">
          ...
          <button onClick={() => setSeleccionadas([])} className="text-slate-400 hover:text-white p-2">
            <X className="w-5 h-5" />
          </button>
        </div>
      )}

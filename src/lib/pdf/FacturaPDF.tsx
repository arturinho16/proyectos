...
const styles = StyleSheet.create({
  ...
  cProd: { width: '10%' }, cCant: { width: '8%' }, cUnidad: { width: '10%' }, cDesc: { flex: 1 },
...
interface Concepto {
  ...
  objetoImpuesto?: string; ivaTasa?: number; iepsImporte?: number; noIdentificacion?: string;
}
        ...
        {factura.conceptos.map((c, i) => {
          ...
          return (
            <View key={i} style={styles.tableRow}>
              ...
              <Text style={styles.cUnidad}>{c.claveUnidad}{c.unidad ? ` - ${c.unidad}` : ''}</Text>
                ...
                {c.objetoImpuesto !== '01' && (
                  <View>
                    ...
                    <Text style={styles.conceptoSub}>
                      IVA: 002, Base: {fmt(c.importe)}, Tasa: {(c.ivaTasa ?? 0.16).toFixed(6)}, Importe: {fmt(ivaImporte)}
                    </Text>
                  </View>
                )}
              </View>
